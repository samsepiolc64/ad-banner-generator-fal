...
CANVAS FILL — NON-NEGOTIABLE: The background color, gradient, or scene MUST extend edge-to-edge across the ENTIRE canvas — from the very top pixel row to the very bottom pixel row, left to right. There must be absolutely no white strips, no grey borders, no empty margins, and no letterboxing of any kind. Every single pixel of the canvas must be filled with the brand background color or a SEAMLESS, UNINTERRUPTED continuation of the same scene.

⚠️ SINGLE UNIFIED IMAGE — CRITICAL: This must be ONE cohesive, seamless image — NOT a composite of multiple images. Do NOT place a different photo crop, a different scene, or a blurred version of the image in the top or bottom areas. The entire canvas must flow as one continuous visual — same lighting, same color grade, same scene extending naturally from top to bottom. Any horizontal band that looks like a separate image = automatic failure.

CONTENT-FREE ZONES (background fills these, content does not): The top 14% of the image must show only the seamless continuation of the main background — same colors, same texture, same lighting as the rest of the scene; no separate photo crop, no blurred band, no different image — no text, no product, no design element may enter this zone. The bottom 33% must similarly show only the seamless background continuation — platform controls cover this area. All visible content (headline, visuals) must be placed in the safe middle zone between these two margins. Place the headline centered horizontally in this safe zone. Design for immediate impact — the message must be understood within one second.`,
