...
export function buildGptImage2Prompt({
  ...
  if (isStories || isTikTokVertical) {
    ...
    textBlock = secondaryLine
      ? `Render this text with crisp Polish typography including all diacritics (ą, ę, ś, ć, ź, ż, ł, ó, ń):\n- PRIMARY HEADLINE (largest, heaviest, dominant — like 700–900 weight at ~80pt on a 1080px canvas): "${primaryLine}"\n- SECONDARY LINE directly below at 55–65% of the primary size, medium weight (400–500): "${secondaryLine}"\n- ⚡ MANDATORY CTA BUTTON (NON-NEGOTIABLE — must appear): a standalone visually distinct button, SOLID fill background color ${ctaHex}, white bold text "${cta}", pill or rounded-rectangle shape, clearly separated below the secondary line — NOT inline text, a real button shape\nFont: ${typographyLine}. Use en dash (–) for any dashes — never em dash (—).`
      : `Render this text with crisp Polish typography including all diacritics (ą, ę, ś, ć, ź, ż, ł, ó, ń):\n- HEADLINE (large, bold): "${primaryLine}"\n- ⚡ MANDATORY CTA BUTTON (NON-NEGOTIABLE — must appear): a standalone visually distinct button, SOLID fill background color ${ctaHex}, white bold text "${cta}", pill or rounded-rectangle shape, clearly separated below the headline — NOT inline text, a real button shape\nFont: ${typographyLine}. Use en dash (–) for any dashes — never em dash (—).`
  }
...
OUTPUT: Sharp, high-resolution, production-ready image that looks like a real published advertisement. Do not render any measurement labels, pixel values, margin indicators, percentage markers, crop marks, dashed zone borders, dimension arrows, or any technical annotations. The final image must be pure ad creative — never a design spec sheet, mood board, or style guide. The background must fill the entire canvas from edge to edge — no white strips, no grey bands, no empty margins, no letterboxing of any kind.${isStories ? ' SINGLE UNIFIED IMAGE — NON-NEGOTIABLE: do NOT create composite images with separate photo crops in the top or bottom areas; do not place a different scene, a blurred version, or a separate image strip in the top 14% or bottom 33% of the canvas — those zones must be a seamless continuation of the same scene, not a separate image.' : ''}`
